...
function updateCountdown(time) {
  ...
  var examDateStart = new Date(currentYear, 0, 1); // 1月1日，月份从0开始计数
  var examDateEnd = new Date(currentYear, 0, 2, 0, 0, 0); // 1月2日 00:00
  var nextYearExamDate = new Date(currentYear + 1, 0, 1); // 下一年的元旦日期

  if (today >= examDateStart && today <= examDateEnd) {
    document.getElementById("countdown").style.display = "none";
    document.getElementById("greeting").style.display = "block";
    document.getElementById("greeting").innerText = "去年已经成为历史，元旦快乐！";
  } else {
